...
- Edit FE qua `/Users/ducnh/Documents/projects/lumora/lumora_fe`, không qua symlink.
...
| 1 | Foundations | Pending | FE + BE | FE/BE chạy local với app/API shell. |
...
- [ ] Confirm Flutter project chạy từ `/Users/ducnh/Documents/projects/lumora/lumora_fe`.
- [ ] Tạo centralized design tokens: warm ivory background, champagne accent, typography, spacing, cards, buttons.
- [ ] Cấu hình routing với auth placeholder và main shell tabs: Today, Journey, Reflection/Review, Settings.
- [ ] Tạo mock data mode để dev không phụ thuộc BE.
- [ ] Tạo reusable base widgets: app scaffold, button, card, loading state, error state, empty state.
- [ ] Cập nhật FE `README.md`: setup, run command, mock mode, troubleshooting.

### BE Tasks

- [ ] Confirm FastAPI project chạy từ `/Users/ducnh/Documents/projects/lumora/lumora_be`.
- [ ] Thêm config/env management và document `.env` keys, không commit secrets.
- [ ] Thêm PostgreSQL connection, ORM baseline, Alembic baseline migration.
- [ ] Thêm `GET /health` trả OK.
- [ ] Thêm CORS config cho mobile development.
- [ ] Chuẩn hóa API error response shape.
- [ ] Cập nhật BE `README.md`: setup, env, migrations, server start, API docs.

### Integration Tasks

- [ ] FE mở placeholder app không cần API.
- [ ] BE health endpoint gọi được local.
- [ ] Brain docs ghi local ports và run commands.

**Verification:**

- [ ] Run `flutter analyze` từ `lumora_fe`.
- [ ] Run `python3 -m py_compile app/main.py` từ `lumora_be`.
- [ ] Mở FastAPI docs local nếu có.
